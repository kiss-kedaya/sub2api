...
spec = importlib.util.spec_from_file_location('guard', Path(__file__).with_name('rollout329_guard.py'))
...
class RolloutTests(unittest.TestCase):
    def setUp(self):
        ...
        self.calls = []
        for name, value in [('ROOT', self.root), ('CONFIG', self.root/'upstream.conf'), ('NAME', 'test'), ('HOST', 'ns575199'), ('STABLE_VERSION', '327'), ('STABLE_PORT', 8099), ('INITIAL_BACKUP_PORT', 8097), ('UI_ENTRY', self.root/'ui'/'index.html')]:
    ...
    def test_health_failure_restores_stable_without_stopping_any_process(self):
        ...
        self.assertNotIn(':8101', guard.CONFIG.read_text())
    ...
    def test_unattributed_upstream_errors_do_not_roll_back(self):
        sample = {'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'business_329': {'200': 35, '503': 15}, 'business_327': {'200': 98, '503': 2}}
    ...
    def test_existing_upstream_errors_do_not_trigger_version_regression(self):
        sample = {'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'business_329': {'200': 97, '503': 3}, 'business_327': {'200': 97, '503': 3}}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value=counts):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
    ...
    def test_percentages_and_backups(self):
        ...
        self.assertIn('10.254.0.2:8099 weight=3960 ', canary)
        self.assertIn('10.254.0.1:8099 weight=5940 ', canary)
        self.assertIn('10.254.0.2:8101 weight=40 ', canary)
        self.assertIn('10.254.0.1:8101 weight=60 ', canary)
        full = guard.config_for(100)
        self.assertIn('10.254.0.2:8101 weight=4000 ', full)
        self.assertIn('10.254.0.1:8101 weight=6000 ', full)
        self.assertEqual(full.count('backup;'), 2)
        self.assertNotIn(':8097', full)

    def test_initial_configs_match_327_full_routes(self):
        old_expected = '''upstream test {
    server 10.254.0.2:8099 weight=4000 max_fails=3 fail_timeout=10s;
    server 10.254.0.1:8099 weight=6000 max_fails=3 fail_timeout=10s;
    server 10.254.0.2:8097 weight=40 max_fails=3 fail_timeout=10s backup;
    server 10.254.0.1:8097 weight=60 max_fails=3 fail_timeout=10s backup;
}
'''
        self.assertEqual(guard.config_for(0), old_expected)
        with patch.object(guard, 'INITIAL_BACKUP_PORT', 8095):
            self.assertEqual(guard.config_for(0), old_expected.replace(':8097', ':8095'))
    ...
    def test_new_ingress_preserves_its_8095_backup(self):
        with patch.object(guard, 'INITIAL_BACKUP_PORT', 8095):
            baseline = guard.config_for(0)
            self.assertIn('10.254.0.2:8099 weight=4000 ', baseline)
            self.assertIn('10.254.0.1:8095 weight=60 max_fails=3 fail_timeout=10s backup;', baseline)
            canary = guard.config_for(1)
            self.assertIn('10.254.0.1:8099 weight=5940 ', canary)
            self.assertIn('10.254.0.2:8101 weight=40 ', canary)
            full = guard.config_for(100)
            self.assertIn('10.254.0.1:8099 weight=60 max_fails=3 fail_timeout=10s backup;', full)
            rollback = guard.rollback_config(['10.254.0.1'])
            self.assertIn('10.254.0.1:8099', rollback)
            self.assertNotIn('10.254.0.2:', rollback)
            self.assertNotIn(':8101', rollback)
    ...
    def test_no_healthy_backup_keeps_route_and_guard(self):
        ...
        with patch.object(guard, 'available_backups', return_value=[]):
            with self.assertRaisesRegex(RuntimeError, 'No healthy 327'):
    ...
    def test_small_failed_sample_blocks_full_promotion(self):
        ...
        guard.save(state)
        with patch.object(guard, 'traffic', return_value={'business_329': {'503': 49}}):
    ...
    def test_slow_recently_finished_error_is_counted(self):
        ...
        log.write_text(json.dumps({'ts': guard.datetime.datetime.fromtimestamp(stamp, guard.datetime.timezone.utc).isoformat(),
                                   'request_time': '240', 'upstream': '10.254.0.1:8101', 'status': '504', 'probe': False})+'\n')
        ...
        self.assertEqual(counts, {'business_329': {'504': 1}})
    ...
    def test_stale_baseline_does_not_claim_version_regression(self):
        ...
        guard.save(state)
        sample = {'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value={'business_329': {'200': 80, '503': 20}}):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
        self.assertEqual(self.calls, [])
    ...
    def test_pending_rollback_rechecks_target_and_restores_previous_route(self):
        ...
        with patch.object(guard, 'run', side_effect=KeyboardInterrupt):
            with self.assertRaises(KeyboardInterrupt):
                guard.rollback('interrupted')
        with patch.object(guard, 'available_backups', return_value=[]):
            with self.assertRaisesRegex(RuntimeError, 'No healthy 327 target during recovery'):
        ...
        with patch.object(guard, 'available_backups', return_value=['10.254.0.2']):
            guard.recover_pending()
        self.assertEqual(guard.get_state()['phase'], 'rolled_back')
        self.assertNotIn('10.254.0.1:8099', guard.CONFIG.read_text())
    ...
    def test_single_healthy_original_is_the_only_rollback_target(self):
        ...
        self.assertIn('10.254.0.2:8099', config)
        self.assertNotIn('10.254.0.1:', config)
        self.assertNotIn(':8095', config)
        self.assertNotIn(':8101', config)
    ...
    def test_one_success_does_not_allow_promotion(self):
        ...
        state.update(percent=1, successful_checks=15, last_check_at=time.time(), config_sha256=guard.fingerprint())
        guard.save(state)
        with patch.object(guard, 'traffic', return_value={'business_329': {'200': 1, '503': 49}}):
            with self.assertRaisesRegex(RuntimeError, 'Insufficient successful business'):
                guard.apply(100, 1)
        self.assertEqual(guard.get_state()['percent'], 1)
        self.assertEqual(self.calls, [])

    def test_retry_is_attributed_to_final_8099_port(self):
        stamp = time.time()
        log = self.root/'retry.log'
        log.write_text(json.dumps({'ts': guard.datetime.datetime.fromtimestamp(stamp, guard.datetime.timezone.utc).isoformat(),
                                   'request_time': '1', 'upstream': '10.254.0.1:8099, 10.254.0.2:8101',
                                   'status': '200', 'probe': False})+'\n')
        with patch.object(guard, 'Path', side_effect=lambda value: log if 'sub2api-rollout.log' in str(value) else Path(value)):
            self.assertEqual(guard.traffic(stamp-180), {'business_329': {'200': 1}})

    def test_client_cancelled_health_probe_does_not_roll_back(self):
        sample = {'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'probe_329': {'200': 1264, '499': 1}}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value=counts):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
        self.assertEqual(self.calls, [])
    ...
    def test_health_server_error_still_rolls_back(self):
        sample = {'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value={'probe_329': {'200': 1264, '503': 1}}):
    ...
    def health_run(self, public_failures=0, origin_failure=False):
        (self.root/'pids.json').write_text(json.dumps({'327': 3270, '329': 3290}))
        calls = []
        def run(*args):
            calls.append(args)
            if args[0] == 'curl':
                url = args[-1]
                if origin_failure and url == 'http://10.254.0.1:8101/readyz':
            ...
            if args[:2] == ('systemctl', 'show'):
                return '3290' if '329' in args[2] else '3270'
    ...
    def test_public_timeout_with_origin_failure_stops_before_public_retry(self):
        ...
        def origin_fails_after_timeout(*args):
            ...
            if public_seen and args[-1] == 'http://10.254.0.1:8101/readyz':
    ...
    def test_resume_from_rollback_route_preserves_previous_phase_files(self):
        ...
        sample = {'rss_kb_327': 1024, 'rss_kb_329': 1024, 'mem_available_kb': 32*1024*1024}
    ...
    def test_rollback_restores_owned_ui_atomically(self):
        ...
        self.assertFalse((guard.UI_ENTRY.parent/'index.html.329-tmp').exists())
