...
ROOT = Path('/opt/sub2api/rollout-329-20260918')
CONFIGS = {
...
STABLE_VERSION = '327'
STABLE_PORT = 8099
INITIAL_BACKUP_PORT = 8095 if HOST == 'ns572368' else 8097
CANDIDATE_VERSION = '329'
CANDIDATE_PORT = 8101
UI_ENTRY = Path('/opt/sub2api/ui/index.html')
...
def config_for(pct):
    ...
    for port, weight in [(STABLE_PORT, 100-pct), (CANDIDATE_PORT, pct)]:
        if weight:
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.329-tmp')
...
class UIAssetParser(HTMLParser):
    def __init__(self):
        super().__init__()
        self.assets = []
...
def ui_health():
    parser = UIAssetParser()
    parser.feed(UI_ENTRY.read_text())
    ui_root = UI_ENTRY.parent.resolve()
    for reference in parser.assets:
        clean = reference.split('?', 1)[0].split('#', 1)[0]
        if clean.startswith(('http://', 'https://', '//')):
            raise RuntimeError('External UI asset is not locally verifiable: '+reference)
        asset = (ui_root/clean.lstrip('/')).resolve()
        if ui_root not in asset.parents or not asset.is_file() or asset.stat().st_size == 0:
            raise RuntimeError('UI asset unavailable: '+reference)


def restore_ui():
    marker = ROOT/'ui-active.json'
    if not marker.exists():
        return
    result = {'at': time.time()}
    try:
        transaction = json.loads(marker.read_text())
        target = Path(transaction['path'])
        before = Path(transaction['before'])
        expected_after = transaction['after_sha256']
        if target != UI_ENTRY:
            raise ValueError('Unexpected UI target')
        before_hash = sha256(before)
        current_hash = sha256(target)
        if current_hash == expected_after:
            atomic_bytes(target, before.read_bytes())
            result.update(status='restored', before_sha256=before_hash, after_sha256=expected_after)
        elif current_hash == before_hash:
            result.update(status='already_before', before_sha256=before_hash, after_sha256=expected_after)
        else:
            result.update(status='external_change_preserved', current_sha256=current_hash,
                          before_sha256=before_hash, after_sha256=expected_after)
    except Exception as exc:
        result.update(status='error', error=type(exc).__name__+': '+str(exc)[:200])
    atomic_text(ROOT/'ui-rollback.json', json.dumps(result))
...
def health(include_old=False):
    ...
    for ip, _ in SHARES:
        for port in ((STABLE_PORT, CANDIDATE_PORT) if include_old else (CANDIDATE_PORT,)):
            ...
            for line in stream:
                try:
                    ...
                    if stamp < cutoff or (started_after is not None and stamp-elapsed < started_after):
                        continue
                    ports = re.findall(r':(8095|8097|8099|8101)\b', upstream)
                    if not ports:
                        continue
                    # Attribute retried requests to their final application, not both versions.
                    version = {'8095': '326', '8097': '326-grok600', '8099': '327', '8101': '329'}[ports[-1]]
...
def platform_faults(state):
    ...
    raw = run('journalctl', '-u', f'sub2api-0.1.{CANDIDATE_VERSION}-canary', '--since', '@'+str(int(since)),
...
def recover_pending():
    ...
    if pending['final_state']['phase'] == 'rolled_back':
        ...
        if not available:
            ...
            state.update(phase='rollback_blocked', armed=True, reason='No healthy 327 target during recovery')
            save(state)
            raise RuntimeError('No healthy 327 target during recovery; previous route restored')
...
def rollback(reason):
    ...
    available = available_backups()
    if not available:
        state.update(phase='rollback_blocked', reason='No healthy 327 target: '+reason)
        save(state)
        raise RuntimeError('No healthy 327 target; keeping current route and guard active')
    after = rollback_config(available)
...
def check():
    ...
    try:
        ...
        sample = metrics()
        if sample['rss_kb_'+CANDIDATE_VERSION] > 17*1024*1024 or sample['mem_available_kb'] < 8*1024*1024:
        ...
        probes = counts.get('probe_'+CANDIDATE_VERSION, {})
...
def apply(pct, previous):
    ...
    if previous == 1:
        ...
        total, _, _ = error_rate(counts.get('business_'+CANDIDATE_VERSION, {}))
        stable_total, _, stable_rate = error_rate(counts.get('business_'+STABLE_VERSION, {}))
        successful = sum(v for k, v in counts.get('business_'+CANDIDATE_VERSION, {}).items() if k.startswith('2'))
...
def main():
    ...
    with open(ROOT/'lock', 'a') as lock:
        ...
        if args.action == 'init':
            ...
            pids = {v: int(run('systemctl', 'show', f'sub2api-0.1.{v}-canary', '-p', 'MainPID', '--value')) for v in (STABLE_VERSION, CANDIDATE_VERSION)}
