...
ROOT = Path('/opt/sub2api/rollout-325-20260916')
CONFIGS = {
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.325-tmp')
...
def check():
    ...
    try:
        health()
        sample = metrics()
        if sample['rss_kb_325'] > 17*1024*1024 or sample['mem_available_kb'] < 8*1024*1024:
...
def apply(pct, previous):
    ...
    if previous == 1:
        ...
        total, _, _ = error_rate(counts.get('business_325', {}))
        stable_total, _, stable_rate = error_rate(counts.get('business_323', {}))
        successful = sum(v for k, v in counts.get('business_325', {}).items() if k.startswith('2'))
...
def main():
    ...
    with open(ROOT/'lock', 'a') as lock:
        ...
        if args.action == 'init':
            ...
            pids = {v: int(run('systemctl', 'show', f'sub2api-0.1.{v}-canary', '-p', 'MainPID', '--value')) for v in ('323', '325')}
