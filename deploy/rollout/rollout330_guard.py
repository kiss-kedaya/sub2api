...
ROOT = Path('/opt/sub2api/rollout-330-20260918')
CONFIGS = {
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.330-tmp')
            ...
            for line in stream:
                try:
                    ...
                    if stamp < cutoff or (started_after is not None and stamp-elapsed < started_after):
                        continue
                    ports = re.findall(r':(8095|8097|8099|8101|8103)\b', upstream)
                    if not ports:
                        continue
                    # Attribute retried requests to their final application, not both versions.
                    version = {'8095': '326', '8097': '326-grok600', '8099': '327', '8101': '329', '8103': '330'}[ports[-1]]
...
def recover_pending():
    ...
    if pending['final_state']['phase'] == 'rolled_back':
        ...
        if not available:
            ...
            state.update(phase='rollback_blocked', armed=True, reason='No healthy 329 target during recovery')
            save(state)
            raise RuntimeError('No healthy 329 target during recovery; previous route restored')
...
def rollback(reason):
    ...
    available = available_backups()
    if not available:
        state.update(phase='rollback_blocked', reason='No healthy 329 target: '+reason)
        save(state)
        raise RuntimeError('No healthy 329 target; keeping current route and guard active')
    after = rollback_config(available)
