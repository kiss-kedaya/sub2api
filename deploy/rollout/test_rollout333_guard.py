...
spec = importlib.util.spec_from_file_location('guard', Path(__file__).with_name('rollout333_guard.py'))
...
class RolloutTests(unittest.TestCase):
    ...
    def test_unattributed_upstream_errors_do_not_roll_back(self):
        sample = {'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'business_333': {'200': 35, '503': 15}, 'business_331': {'200': 98, '503': 2}}
    ...
    def test_existing_upstream_errors_do_not_trigger_version_regression(self):
        sample = {'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'business_333': {'200': 97, '503': 3}, 'business_331': {'200': 97, '503': 3}}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value=counts):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
...
'''
    ...
    def test_small_failed_sample_blocks_full_promotion(self):
        ...
        guard.save(state)
        with patch.object(guard, 'traffic', return_value={'business_333': {'503': 49}}):
    ...
    def test_slow_recently_finished_error_is_counted(self):
        ...
        self.assertEqual(counts, {'business_333': {'504': 1}})
    ...
    def test_stale_baseline_does_not_claim_version_regression(self):
        ...
        guard.save(state)
        sample = {'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value={'business_333': {'200': 80, '503': 20}}):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
        self.assertEqual(self.calls, [])
    ...
    def test_one_success_does_not_allow_promotion(self):
        ...
        state.update(percent=1, successful_checks=15, last_check_at=time.time(), config_sha256=guard.fingerprint())
        guard.save(state)
        with patch.object(guard, 'traffic', return_value={'business_333': {'200': 1, '503': 49}}):
            with self.assertRaisesRegex(RuntimeError, 'Insufficient successful business'):
                guard.apply(100, 1)
        self.assertEqual(guard.get_state()['percent'], 1)
        self.assertEqual(self.calls, [])
    ...
    def test_retry_is_attributed_to_final_8107_port(self):
        ...
        with patch.object(guard, 'Path', side_effect=lambda value: log if 'sub2api-rollout.log' in str(value) else Path(value)):
            self.assertEqual(guard.traffic(stamp-180), {'business_333': {'200': 1}})

    def test_client_cancelled_health_probe_does_not_roll_back(self):
        sample = {'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
        counts = {'probe_333': {'200': 1264, '499': 1}}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value=counts):
            guard.check()
        self.assertTrue(guard.get_state()['armed'])
        self.assertEqual(self.calls, [])
    ...
    def test_health_server_error_still_rolls_back(self):
        sample = {'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
        with patch.object(guard, 'health'), patch.object(guard, 'metrics', return_value=sample), patch.object(guard, 'traffic', return_value={'probe_333': {'200': 1264, '503': 1}}):
    ...
    def health_run(self, public_failures=0, origin_failure=False):
        (self.root/'pids.json').write_text(json.dumps({'331': 3310, '333': 3330}))
        calls = []
        def run(*args):
            calls.append(args)
            if args[0] == 'curl':
            ...
            if args[:2] == ('systemctl', 'show'):
                return '3330' if '333' in args[2] else '3310'
    ...
    def test_resume_from_rollback_route_preserves_previous_phase_files(self):
        ...
        sample = {'rss_kb_331': 1024, 'rss_kb_333': 1024, 'mem_available_kb': 32*1024*1024}
    ...
    def test_rollback_restores_owned_ui_atomically(self):
        ...
        self.assertFalse((guard.UI_ENTRY.parent/'index.html.333-tmp').exists())
