...
ROOT = Path('/opt/sub2api/rollout-326-grok600-20260916')
CONFIGS = {
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.326-grok600-tmp')
...
def health(include_old=False):
    ...
    for ip, _ in SHARES:
        for port in ((8095, 8097) if include_old else (8097,)):
    ...
    for version, pid in json.loads((ROOT/'pids.json').read_text()).items():
        if version == '326' and not include_old:
            continue
    ...
    for version, pid in json.loads((ROOT/'pids.json').read_text()).items():
        try:
            ...
            if version != '326':
                raise
            result['rss_kb_326'] = 0
            result['original_326_pid_missing'] = True
            continue
            ...
            for line in stream:
                try:
                    ...
                    if stamp < cutoff or (started_after is not None and stamp-elapsed < started_after):
                        continue
                    ports = re.findall(r':(8091|8095|8097)\b', upstream)
                    if not ports:
                        continue
                    # Attribute retried requests to their final application, not both versions.
                    version = {'8091': '323', '8095': '326', '8097': '326-grok600'}[ports[-1]]
...
def available_backups():
    ...
    for ip, _ in SHARES:
        try:
            if json.loads(run('curl', '-fsS', '-m', '3', f'http://{ip}:8095/readyz')).get('status') == 'ready':
                available.append(ip)
        except Exception:
            pass
    return available
...
def rollback_config(available):
    return '\n'.join(line for line in config_for(0).splitlines()
                     ...
                     (':8095 ' not in line or any(f'{ip}:8095 ' in line for ip in available)))+'\n'
...
def recover_pending():
    ...
    if pending['final_state']['phase'] == 'rolled_back':
        ...
        if not available:
            ...
            state.update(phase='rollback_blocked', armed=True, reason='No healthy 326 target during recovery')
            save(state)
            raise RuntimeError('No healthy 326 target during recovery; previous route restored')
...
def rollback(reason):
    ...
    available = available_backups()
    if not available:
        state.update(phase='rollback_blocked', reason='No healthy 326 target: '+reason)
        save(state)
        raise RuntimeError('No healthy 326 target; keeping current route and guard active')
    after = rollback_config(available)
...
def check():
    ...
    try:
        health()
        sample = metrics()
        if sample['rss_kb_326-grok600'] > 17*1024*1024 or sample['mem_available_kb'] < 8*1024*1024:
        ...
        probes = counts.get('probe_326-grok600', {})
...
def apply(pct, previous):
    ...
    if previous == 1:
        ...
        total, _, _ = error_rate(counts.get('business_326-grok600', {}))
        stable_total, _, stable_rate = error_rate(counts.get('business_326', {}))
        successful = sum(v for k, v in counts.get('business_326-grok600', {}).items() if k.startswith('2'))
    ...
    sample = metrics()
    if sample['mem_available_kb'] < sample['rss_kb_326']+8*1024*1024:
...
def main():
    ...
    with open(ROOT/'lock', 'a') as lock:
        ...
        if args.action == 'init':
            ...
            pids = {v: int(run('systemctl', 'show', f'sub2api-0.1.{v}-canary', '-p', 'MainPID', '--value')) for v in ('326', '326-grok600')}
            ...
            _, _, baseline = error_rate(baseline_counts.get('business_326', {}))
