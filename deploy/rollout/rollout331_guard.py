...
ROOT = Path('/opt/sub2api/rollout-331-20260918')
CONFIGS = {
...
STABLE_VERSION = '330'
STABLE_PORT = 8103
INITIAL_BACKUP_PORT = 8101
CANDIDATE_VERSION = '331'
CANDIDATE_PORT = 8105
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.331-tmp')
            ...
            for line in stream:
                try:
                    ...
                    if stamp < cutoff or (started_after is not None and stamp-elapsed < started_after):
                        continue
                    ports = re.findall(r':(8095|8097|8101|8103|8105)\b', upstream)
                    if not ports:
                        continue
                    # Attribute retried requests to their final application, not both versions.
                    version = {'8095': '326', '8097': '326-grok600', '8101': '329', '8103': '330', '8105': '331'}[ports[-1]]
...
def recover_pending():
    ...
    if pending['final_state']['phase'] == 'rolled_back':
        ...
        if not available:
            ...
            state.update(phase='rollback_blocked', armed=True, reason='No healthy 330 target during recovery')
            save(state)
            raise RuntimeError('No healthy 330 target during recovery; previous route restored')
...
def rollback(reason):
    ...
    available = available_backups()
    if not available:
        state.update(phase='rollback_blocked', reason='No healthy 330 target: '+reason)
        save(state)
        raise RuntimeError('No healthy 330 target; keeping current route and guard active')
    after = rollback_config(available)
