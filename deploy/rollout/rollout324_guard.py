...
ROOT = Path('/opt/sub2api/rollout-324-20260916')
CONFIGS = {
...
def atomic_text(target, value):
    stage = target.with_name(target.name+'.324-tmp')
        ...
        with path.open() as stream:
            ...
            for line in stream:
                try:
                    ...
                    version = {'8090': '322', '8091': '323', '8092': '324'}[ports[-1]]
...
def check():
    ...
    try:
        health()
        sample = metrics()
        if sample['rss_kb_324'] > 17*1024*1024 or sample['mem_available_kb'] < 8*1024*1024:
...
def apply(pct, previous):
    ...
    if previous == 1:
        ...
        total, _, _ = error_rate(counts.get('business_324', {}))
        stable_total, _, stable_rate = error_rate(counts.get('business_323', {}))
        successful = sum(v for k, v in counts.get('business_324', {}).items() if k.startswith('2'))
...
def main():
    ...
    with open(ROOT/'lock', 'a') as lock:
        ...
        if args.action == 'init':
            ...
            pids = {v: int(run('systemctl', 'show', f'sub2api-0.1.{v}-canary', '-p', 'MainPID', '--value')) for v in ('323', '324')}
