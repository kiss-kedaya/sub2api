...
func (r *scheduledTestPlanRepository) Create(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		INSERT INTO scheduled_test_plans (account_id, model_id, prompt_text, cron_expression, enabled, max_results, auto_recover, next_run_at, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())
		RETURNING id, account_id, model_id, prompt_text, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
	`, plan.AccountID, plan.ModelID, plan.PromptText, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.NextRunAt)
	return scanPlan(row)
}
...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		SET model_id = $2, prompt_text = $3, cron_expression = $4, enabled = $5, max_results = $6, auto_recover = $7, next_run_at = $8, updated_at = NOW()
		WHERE id = $1
		RETURNING id, account_id, model_id, prompt_text, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
	`, plan.ID, plan.ModelID, plan.PromptText, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.NextRunAt)
	return scanPlan(row)
}
...
func scanPlan(row scannable) (*service.ScheduledTestPlan, error) {
	...
	if err := row.Scan(
		&p.ID, &p.AccountID, &p.ModelID, &p.PromptText, &p.CronExpression, &p.Enabled, &p.MaxResults, &p.AutoRecover,
