...
// GetOpenAICodexTicketEnabledSnapshot is request-path safe: it never waits for
// settings storage. A stale or missing cache is refreshed asynchronously so a
// setting changed on another instance becomes visible on the next refresh.
func (s *SettingService) GetOpenAICodexTicketEnabledSnapshot(fallback bool) bool {
	...
	go func() { <-resultCh }()
	// No fresh snapshot means fail closed for this security-sensitive feature.
	// The async refresh will make an explicit remote enable visible on a later
	// request without adding a storage read to the gateway hot path.
	return false
}
...
func (s *SettingService) GetOpenAICodexTicketHarvestProxyURLWithPresence(ctx context.Context) (string, bool) {
	...
	resultCh := s.openAICodexTicketHarvestProxySF.DoChan(SettingKeyOpenAICodexTicketHarvestProxyURL, func() (any, error) {
		...
		if err != nil && !errors.Is(err, ErrSettingNotFound) {
			...
			last, _ := s.openAICodexTicketHarvestProxyCache.Load().(*cachedOpenAICodexTicketHarvestProxy)
			if last != nil {
				value = last.value
			}
			configured := last != nil && last.configured
			...
			return cachedOpenAICodexTicketHarvestProxy{value: value, configured: configured}, nil
		}
