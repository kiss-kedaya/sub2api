...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string, opts ...AccountTestOptions) error {
	...
	if account.IsOpenAI() || account.IsGeminiOpenAIProtocol() {
		return s.testOpenAIAccountConnection(c, account, modelID, prompt, normalizeAccountTestMode(mode))
	}
...
func (s *AccountTestService) testBedrockAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string, prompt string) error {
	...
	resp, err := s.doTestRequest(req, proxyURL, account)
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Request failed: %s", err.Error()))
	}
	defer func() { _ = resp.Body.Close() }()
...
func (s *AccountTestService) resolveTestTLSProfile(account *Account) *tlsfingerprint.Profile {
...
func (s *AccountTestService) doTestRequest(req *http.Request, proxyURL string, account *Account) (*http.Response, error) {
	return s.httpUpstream.DoWithTLS(req, proxyURL, account.ID, account.Concurrency, s.resolveTestTLSProfile(account))
}

func (s *AccountTestService) grokTestDo(req *http.Request, account *Account) (*http.Response, error) {
	return s.doTestRequest(req, s.grokTestProxyURL(account), account)
}
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID string) error {
	...
	resp, err := s.grokTestDo(req, account)
	if err != nil {
...
func (s *AccountTestService) testGrokImageGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt, imageDataURL string) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		resp, doErr = s.grokTestDo(req, account)
		if doErr == nil {
			break
		}
...
func (s *AccountTestService) testGrokVideoGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt string, opts AccountTestOptions) error {
	...
	for time.Now().Before(deadline) {
		...
		statusResp, err := s.grokTestDo(statusReq, account)
		if err != nil {
...
func (s *AccountTestService) RunTestBackground(ctx context.Context, accountID int64, modelID string, prompts ...string) (*ScheduledTestResult, error) {
	...
	prompt := DefaultScheduledTestPrompt
	if len(prompts) > 0 && strings.TrimSpace(prompts[0]) != "" {
		prompt = strings.TrimSpace(prompts[0])
	}
